"""Fermentation chamber supervisor snapshot and supervised apply bridge."""
...
def _attr(hass: HomeAssistant, entity_id: str, attr: str) -> Any:
    state = hass.states.get(entity_id)
    return state.attributes.get(attr) if state is not None else None
...
def build_fermentation_climate_supervisor_snapshot(hass: HomeAssistant) -> dict[str, Any]:
    ...
    snapshot = {
        ...
        "source": "python_fermentation_chamber_supervisor",
