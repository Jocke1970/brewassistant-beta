"""BrewZilla source-scoped authority at the normal BA actuator entrypoints.

This is an intentionally narrow integration gate, not a claim that all legacy
writer paths have been audited. Keep PR in draft until every direct service/
button path and supervised execution path has regression coverage.
"""
...
def _write_allowed(hass, entity: str, *, switch_action: str | None = None, value: float | None = None) -> bool:
    ...
    if authority.may_write_brewzilla is True or authority.may_write_brewzilla is None:
        return True  # Preserve separately governed Manual mode for this PR.
...
async def _policy_execute(hass, action):
    ...
    if entity in BREWZILLA_ENTITIES:
        ...
        if not _write_allowed(hass, entity, switch_action=switch_action, value=number):
            return control_policy._store_policy_result(hass, {
                **action, "status": "source_authority_denied",
                "summary": "BrewZilla command refused: process source has no write authority.",
            })
...
def install_source_authority_runtime():
    ...
    global _PREVIOUS_STOP, _PREVIOUS_SUPERVISED_BUILD
    if _INSTALLED:
        return
