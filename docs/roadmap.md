...
- **Grainfather GF30:** beta.15-kandidaten har read-only cloud discovery, persistent Pill/manuell preflight, passiv rate-learning, dual Pill/internal safe-point och valfri coolant/freezer/thermostat-telemetri. Dynamisk framtida coolant-target är beslutad som GF30-target minus adaptivt headroom inom verifierade min/max-gränser; GF30 äger fortsatt pumpen och `generic_thermostat` frysen. Första nattens separata frys/vatten-baseline 26/9 ogiltigförklarades för thermal learning eftersom den nya YAML-termostaten inte hade lästs om och vattnet frös; detta är nu en explicit reload/runtime-preflight-grind. Ingen Grainfather-service call, pump-, frys- eller coolant-setpoint-write är godkänd; live GF30 och fysisk fail-safe återstår. [Fältnotering 26/9](gf30-coolant-field-note-2026-09-26.md).
